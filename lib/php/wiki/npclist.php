...
function get_npc_type($type, $unangreifbar) {
    $type_clean = strtolower(str_replace('-', '', $type));
    
    if ($unangreifbar === false) {
        switch ($type_clean) {
            case 'gruppe':
            case 'gruppen':
    ...
    if (!in_array($type_clean, $types)) {
        return array_search('unbekannt', $types);
    } else {
        return array_search($type_clean, $types);
    }
}
...
foreach ($page_chunks as $pages) {
    ...
    foreach ($pages_sorted as $pageid => $info) {
        ...
        # NPC Vorlage
        $base = array_map('parse_template', get_templates('NPC/Layout', $raw))[0];
        ...
        $base_entry = array(
            ...
            'typ' => get_npc_type($base['Typ'], $unangreifbar)
        );
        fputcsv($out, $base_entry);
