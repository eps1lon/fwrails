...
if (isset($_GET['evaluate'])) {
    ...
    if (isset($_GET['name'])) {
        ...
        $groups['AX V3'] = "name IN ('" . implode("', '", array_map("utf8_decode", array(
            'Fleckfarbenfisch',
            'Tiefsee-Aal',
            'Röhrenkrebs',
            'dreiköpfige Wasserschlange'
        ))) . "') OR (name = 'Rotzahnhai' AND pos_x < 0)";
        $groups['AX VX'] = "!(" . $groups['A3 V3'] . ") AND !(" . $groups['AX V3'] . ") AND id NOT BETWEEN 323 AND 343 AND id NOT IN (633, 865, 1572)";
    ...
    $sql_query = "SELECT GROUP_CONCAT(id SEPARATOR ',') as  ids, SUM(chasecount) AS chasecount, strength ".
                 "FROM npcs WHERE id > 0 GROUP BY strength";
    ...
    while ($npc = mysql_fetch_assoc($npcs)) {
        ...
        echo "<tr><td>" . $npc['strength'] . "</td><td>" . (int)$drops['count'] . "</td>".
