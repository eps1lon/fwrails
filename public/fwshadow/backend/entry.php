    ...
    if ($length > 0) {
        ...
        foreach ($log as $i => $entry) {
            if (isset($entry->item)) { // pflanze
                ...
                $sql_query = "INSERT IGNORE INTO items_places (item_id, place_id) VALUES ".
                             "('$item_id', (SELECT id FROM places WHERE ".
                             "pos_x = '" . $entry->place->x . "' AND pos_y = '" . $entry->place->y . "'))";
                mysql_query($sql_query, $db);
        ...
        $sql_query = "REPLACE INTO passages (from_x, from_y, to_x, to_y, via) ".
